...
public class CollectionDemo01 {
	public static void main(String[] args) {
		...
		coll.add("abc");
		coll.add("def");
		coll.add(123);//int类型可以自动装箱为Integer类型,Integer类型向上转型为Object类型
		coll.add(123);
		...
		//删除集合中指定单个对象,注意:即使有删除的对象有重复元素,一个remove()只能删除一个对象
		coll.remove(123);
		coll.remove(123);
		...
		si.toString();//编译没有报错,父接口中存在这个toStirng();
	}
}
